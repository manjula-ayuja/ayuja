...
import {Box,Typography,Card,CardContent,Avatar,IconButton,TextField,Button,
    Snackbar,Alert,Dialog,DialogTitle,
    DialogContent,DialogActions,Link,InputAdornment,MenuItem,
} from "@mui/material";
...
useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        if (res.ok) {
          // Convert emergency_contacts and family_members to objects
  ...
  const validateDocument = (type, number) => {
    if (type === "Aadhar") {
      return /^\d{12}$/.test(number); // 12 digits
    }
    if (type === "PAN") {
      return /^[A-Z]{5}[0-9]{4}[A-Z]{1}$/.test(number); // PAN format
    }
    return true;
  };

  const handleSaveProfile = async () => {
    // validate docs before sending
    ...
    <Box sx={{ maxWidth: 500, mx: "auto", mt: 5,mb:10 }}>
        ...
        <CardContent>
          {editMode ? (
            <>
              ...
              <Button  sx={{color:"#006D77"}} onClick={() => setFormData({ ...formData, emergency_contacts: [...formData.emergency_contacts, { name: "", phone: "" }] })}>
                + Add Contact
              </Button>
              ...
              <Button sx={{color:"#006D77"}} onClick={() => setFormData({ ...formData, family_members: [...formData.family_members, { name: "", relation: "" }] })}>
                + Add Member
              </Button>
              ...
              {formData.documents.map((d, idx) => (
                <Box key={idx} sx={{ display: "flex", gap: 2, mt: 1 }}>
                  ...
                  <TextField
                    label="Number"
                    value={d.number}
                    onChange={(e) => handleDocumentChange(idx, "number", e.target.value)}
                    fullWidth
                  />
                </Box>
              ))}
              <Button sx={{color:"#006D77"}} onClick={() => setFormData({ ...formData, documents: [...formData.documents, { type: "Aadhar", number: "" }] })}>
                + Add Document
              </Button>

              <Button variant="contained" fullWidth sx={{ mt: 2,backgroundColor:"#006D77" }} onClick={handleSaveProfile}>
                Save Changes
              </Button>
            </>
          ) : (
      ...
      <Dialog open={passwordDialog} onClose={() => setPasswordDialog(false)}>
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={handleChangePassword} sx={{backgroundColor:"#006D77"}}>
            Update
          </Button>
        </DialogActions>
      </Dialog>
